...
export default function App() {
  ...
  return (
    <div
      className={`flex min-h-screen ${fontFamily} select-none antialiased transition-colors bg-surface text-app`}
    ...
    >
      ...
      {/* ═══════════════════════════════════════════
           ...
           ═══════════════════════════════════════════ */}
      <aside
        className="hidden md:flex flex-col md:w-20 lg:w-64 py-6 fixed inset-y-0 left-0 z-50 border-r border-app bg-surface transition-all duration-300"
      >
        <div
          className="flex items-center md:justify-center lg:justify-start gap-3 px-0 lg:px-6 pb-6 border-b border-app mb-6"
        >
          <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-emerald-600 to-teal-600 text-white flex items-center justify-center shadow-md shrink-0">
            <Anchor className="w-5 h-5 text-white" strokeWidth={2.5} />
          </div>
          <span
            className="font-bold tracking-tight leading-none text-base hidden lg:block text-app"
          >
            Seafarer Calc
          </span>
        </div>
        ...
        <nav className="flex-1 px-3 lg:px-4 space-y-1.5 overflow-x-hidden">
            ...
            return (
              <button
                ...
                className={`w-full flex items-center md:justify-center lg:justify-start gap-3 px-3 lg:px-4 py-2.5 rounded-xl font-medium text-xs tracking-wide transition-all border-none text-left cursor-pointer ${
                  isActive
                    ? "bg-emerald-50 text-emerald-800 lg:border-l-4 lg:border-l-emerald-600 lg:pl-3.5 font-bold"
                    : "text-muted-app hover:bg-recessed hover:text-app"
                }`}
                title={item.label}
              >
                <Icon
                  className={`w-5 h-5 shrink-0 ${isActive ? "text-emerald-700" : "text-muted-app"}`}
                />
          ...
          <button
            ...
            className="w-full flex items-center md:justify-center lg:justify-between px-3 lg:px-4 py-2.5 rounded-xl font-medium text-xs tracking-wide transition-all border border-app bg-recessed hover:border-slate-300 text-app mt-2 cursor-pointer"
            title={cloudAccount ? "Cloud Synced" : "Guest Mode"}
          >
            <div className="flex items-center gap-2.5">
              <Cloud
                className={`w-5 h-5 shrink-0 ${cloudAccount ? "text-emerald-600" : "text-amber-600"}`}
              />
            ...
            <span
              className="hidden lg:block text-[10px] px-1.5 py-0.5 rounded border border-app bg-surface text-muted-app"
            >
        ...
        <div
          className="px-2 lg:px-4 border-t border-app pt-5 mt-auto relative"
        >
          <button
            onClick={() => setCurrentPage("about")}
            className="w-full flex items-center md:justify-center lg:justify-start gap-3 px-2 lg:px-3 py-2 border border-app bg-recessed hover:bg-surface text-app rounded-xl transition-all cursor-pointer text-left font-sans select-none"
            title={activeProfile.name || "Seafarer"}
          >
            <div
              className="w-8 h-8 rounded-lg flex items-center justify-center shrink-0 bg-surface border border-app"
            >
              <User
            ...
            <div className="hidden lg:block min-w-0 flex-1">
              <div
                className="text-xs font-bold truncate text-app"
              >
                {activeProfile.name || "Seafarer"}
              </div>
              <div className="text-[10px] text-muted-app truncate">
                {activeProfile.rank || "Officer"}
              </div>
            </div>
            <span className="hidden lg:block text-muted-app text-xs shrink-0">
      ...
      <main className="flex-grow md:pl-20 lg:pl-64 min-h-screen relative flex flex-col justify-start pb-20 md:pb-8 w-full max-w-full overflow-x-hidden transition-all duration-300">
        ...
        >
          <div className="flex items-center gap-2.5">
            <div className="w-7 h-7 rounded-lg bg-emerald-600 text-white flex items-center justify-center shadow-md shrink-0">
              <Anchor className="w-4 h-4 text-white" strokeWidth={2.5} />
            </div>
            <div>
              <span
                className="font-bold text-xs tracking-tight block leading-tight text-app"
              >
                Seafarer Calc
              </span>
              <span
                className="text-[10px] block leading-tight text-muted-app"
              >
                NRI &amp; Seatime
              </span>
            </div>
            ...
            <button
              ...
              className={`btn-neu px-2.5 py-1 rounded-full text-[11px] font-semibold flex items-center gap-1.5 transition-all border cursor-pointer ${
                cloudAccount
                  ...
                  : "bg-recessed border-app text-app hover:bg-surface"
              }`}
            >
              <Cloud
                className={`w-3.5 h-3.5 ${cloudAccount ? "text-emerald-600" : "text-amber-600"}`}
              />
        ...
        {/* ═══════════════════════════════════════════
             ...
             ═══════════════════════════════════════════ */}
        <nav
          className="md:hidden fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-md h-16 z-50 flex items-center justify-around px-1 pb-safe border-t border-app bg-surface text-app shadow-lg transition-colors"
        >
          {NAV_ITEMS.filter(
            (item) =>
              item.id !== "admin" ||
              ...
              <button
                ...
                className={`flex flex-col items-center justify-center gap-0.5 flex-1 h-full border-none bg-transparent cursor-pointer transition-all ${
                  isActive
                    ...
                    : "text-muted-app hover:text-app font-medium"
                }`}
              >
                <Icon
                  className={`w-4.5 h-4.5 min-[375px]:w-5 min-[375px]:h-5 ${isActive ? "stroke-emerald-600" : "stroke-current text-muted-app"}`}
                  strokeWidth={2.2}
                />
